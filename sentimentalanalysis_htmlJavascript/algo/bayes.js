...
var Storage = (function (Storage) {
  ...
  Storage.getItem = function (k) {
    if (typeof Storage._data[k] === "undefined") return null;
...
var Bayes = (function (Bayes) {
  ...
  Bayes.train = function (text, label) {
    ...
    for (var i = 0; i < length; i++) incrementStem(words[i], label);
    ...
    for (var j = 0; j < labels.length; j++) {
      ...
      for (var i = 0; i < length; i++) {
        ...
        if (_stemTotalCount === 0) {
          ...
          wordicity =
            (3 * 0.5 + _stemTotalCount * wordicity) / (3 + _stemTotalCount);

          if (wordicity === 0) wordicity = 0.00001;
          else if (wordicity === 1) wordicity = 0.99999;
        }
